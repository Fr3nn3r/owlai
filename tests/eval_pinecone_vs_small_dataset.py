...
QUESTIONS_TO_TEST = FRENCH_LAW_QUESTIONS["tax"]
...
def main():
    ...
    REPORTS_DIR = "data/evaluation/pinecone-v0.3"
    ...
    tools = [
        Pinecone_RAG_Tool(**TOOLS_TO_TEST["pinecone_french_law_lookup"]),
        FAISS_RAG_Tool(**TOOLS_TO_TEST["rag-fr-tax-law-v1"]),
    ]
    ...
    for question_idx, question in enumerate(QUESTIONS_TO_TEST):
        ...
        for tool_to_test in tools:
            ...
            try:
                full_rag_answer = tool_to_test.get_rag_resources(question)
                # sprint(full_rag_answer)

                if "reranked_docs" in full_rag_answer["metadata"]:
                    reranked_docs_metadata = full_rag_answer["metadata"][
                        "reranked_docs"
                    ]
                else:
                    reranked_docs_metadata = []

                if "retrieved_docs" in full_rag_answer["metadata"]:
                    retrieved_docs_metadata = full_rag_answer["metadata"][
                        "retrieved_docs"
                    ]
                else:
                ...
                for idoc_meta in tqdm(
                    ...
                    chunk_content = tool_to_test.get_document_content_by_id(
                        idoc_meta["id"]
                    )
                        ...
                        all_docs.append(
                            {
                                ...
                                "reranker_score": (
                                    ...
                                    if "rerank_score" in idoc_meta
                                    and idoc_meta["rerank_score"] != "Unknown"
                                    else None
                                ),
                                "is_reranked": True,
